...
class MicroOrderController extends Controller
{
    ...
    public function seconds()
    {
        $seconds = MicroSecond::where('status', 1)
            ->get();
        $fee=Currency::with([])->where('id',1)->first()->micro_trade_fee;
        return $this->success('',0,['data'=>$seconds,'fee'=>$fee]);
    }
    ...
    public function getResult(Request $request) {
        ...
        if ($id) {
            ...
            return $this->success('',0,$order);//就是这个
        }
    ...
    public function submit(Request $request)
    {
        ...
        Cache::put("microtrade_$user_id", 1, Carbon::now()->addSeconds(3));//用户1秒只能点击一次
        try {
            //进行基本验证
            ...
            $market = MarketHour::getEsearchMarketById($currency_match->currency_name, $currency_match->legal_name, '1min', strtotime(date('Y-m-d H:i')));
