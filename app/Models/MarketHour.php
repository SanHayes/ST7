...
class MarketHour extends Model
{
    ...
    public static function getEsearchMarket($base_currency, $quote_currency, $peroid, $from, $to)
    {
        ...
        $datakline = file_get_contents($url);
        $kline_data = json_decode($datakline, true);
    ...
    public static function getEsearchMarketXNB($base_currency, $quote_currency, $peroid, $from, $to)
    {
        ...
        $res = file_get_contents("https://api.huobi.pro/market/history/kline?symbol=" . strtolower($base_currencys . $quote_currencys) . "&period={$peroid}&size={$size}");
        // die("https://api.huobi.pro/market/history/kline?symbol=" . strtolower($base_currencys . $quote_currencys) . "&period={$peroid}&size=500");
